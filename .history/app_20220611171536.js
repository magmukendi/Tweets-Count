...
function colorChange(){
        ...
        if(s.match(char).length <=145/2){
                ...
                p.style.color ='green'
                p.style = 'font-weight:bold; font-size:60rem; margin-top:5px; flex-wrap:wrap';
                number.style.color = 'green'
        }
        else if(s.match(char).length >=145/2 && s.match(char).length <135){
                p.style = 'font-weight:bold; font-size:45rem; margin-top:5px; flex-wrap:wrap';
                p.style.color = '#1DA1F2';


        }
        else if(s.match(char).length >=135 && s.match(char).length<=145){
                p.style = 'font-weight:bold; font-size:25rem; margin-top:5px; flex-wrap:wrap';
                p.style.color ='red';
